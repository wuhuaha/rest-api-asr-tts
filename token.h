...
extern const char BEEBOT_TOKEN_URL[];
extern const int MAX_TOKEN_SIZE;
extern const int MAX_CUID_SIZE;

/*
 * @brief 从beebot网站获取token ,调用 parse_token解析结果。
 * @param token out 获取的token
 * @param cuid  out 获取的cuid
 * @return RETURN_OK 成功 其它失败
 */
RETURN_CODE beebot_get_token(char *token, char *cuid);
